...
class AppProvider extends ChangeNotifier {
  ...
  bool       _isInitializing = true;  // ✅ FIX: tracks first auth resolution
  String?    _error;

  UserModel?       get currentUser    => _currentUser;
  bool             get isLoading      => _isLoading;
  bool             get isInitializing => _isInitializing;  // ✅ exposed
  ...
  // ── Init ───────────────────────────────────────────────────────────────────
  // ✅ FIX: Only call this ONCE (from main.dart). SplashScreen no longer
  //         calls init() again — that was registering the listener twice.
  void init() {
    FirebaseAuth.instance.authStateChanges().listen((user) {
      if (user != null) {
        ...
        _currentUser    = null;
        _isInitializing = false;  // ✅ auth state resolved (logged out)
        notifyListeners();
      }
    });
  }
  ...
  void _listenToUser(String uid) {
    _authService.userStream(uid).listen((user) {
      _currentUser    = user;
      _isInitializing = false;  // ✅ first user data received
      notifyListeners();
    });
  }
