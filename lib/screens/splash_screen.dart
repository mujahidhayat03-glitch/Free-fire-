...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _navigate() async {
    // ✅ FIX #1: Do NOT call provider.init() here — it is already called in
    //    main.dart via AppProvider()..init(). A second call registers the
    //    authStateChanges listener twice, causing duplicate events.

    // ✅ FIX #2: Wait for isInitializing to become false before navigating.
    //    Previously, currentUser was checked immediately after init(), but the
    //    stream had not fired yet — so currentUser was always null and every
    //    user was sent to LoginScreen even when already logged in.
    final provider = context.read<AppProvider>();

    // Enforce minimum splash duration (UX) while auth resolves in background
